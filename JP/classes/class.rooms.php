...
class Rooms{
	...
	public function list_rooms_search($keyword){
		
		//$keyword = "%".$keyword."%";

		$q = $this->conn->prepare('SELECT * FROM tbl_rooms WHERE room_no LIKE ?');
	...
	public function update_rooms($room_no,$room_price,$room_status, $room_vacancy){
		...
		$sql = "UPDATE tbl_rooms SET room_no=:room_no,room_price=:room_price,room_status=:room_status, room_vacancy=:room_vacancy WHERE room_no=:room_no";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':room_no'=>$room_no, ':room_price'=>$room_price,':room_status'=>$room_status, 'room_vacancy'=>$room_vacancy));
		return true;
	}
